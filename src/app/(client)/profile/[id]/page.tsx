...
const Orders = async ({ id }: { id: number }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <div>
        <h3 className="mb-6 scroll-m-20 text-2xl text-center font-semibold tracking-tight">Mis Compras</h3>
      </div>
      <Table className="mb-8">
        ...
        <TableBody>
          {orders && orders.length > 0 ? (
            orders
              .filter((order) => order.product && order.product.title) // Filtrar orders sin product o sin title
              .map(
                ({
                  product: { title, id } = {}, // Valor por defecto para evitar errores
                  purchasedAt,
                  status,
                }) => (
    ...
    <>
      ...
      >
        ...
        <div className="grid w-full sm:w-full items-center gap-1">
          ...
          <Input id="streetNumber" name="streetNumber" type="number" defaultValue={user.streetNumber ?? ""} />
        </div>

        <div className="grid w-full sm:w-full items-center gap-1">
          <Label htmlFor="indications" className="mb-2">
            Indicaciones
          </Label>
          <Input
            id="indications"
